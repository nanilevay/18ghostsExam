...
public class BoardManager : MonoBehaviour, IGameController
{
    ...
    /// <summary>
    /// This method allows us to set a tile in its chosen place
    /// </summary>
    ...
    public virtual void PlacePiece(IMapElement ChosenTile)
    {
        ...
        // Check if the current piece on tile is null
        if (ChosenTile.PieceOnTile == null)
        {
            // check if the player is on a mirror
            if (CurrentPlayer.ChosenPiece.OnMirror)
            {
                // Check if the player clicked a mirror and move
            ...
            // Check if the piece is in the dungeon or start
            else if (CurrentPlayer.ChosenPiece.inDungeon || CurrentPlayer.start)
            {
                // Check if the chosen piece matches the players'colour
                if (ChosenTile.colour == CurrentPlayer.ChosenPiece.colour)
                {
                    (CurrentPlayer.ChosenPiece as MonoBehaviour)
                        .transform.position =
            ...
            // if the player is neither on a mirror or a dungeon
            else
            {
                // If the move was valid
                if (CurrentMoveIsValid)
                {
                    // If the player was on a mirror and is teleporting
                    if (ChosenTile is Mirror &&
                    ...
                    // if the player is on any other tile
                    else
                    {
                        PreviousTile.PieceOnTile = null;
        ...
        // If the piece on tile isn't null
        else
        {
            ...
            // If the move is none of the valid ones
            else
